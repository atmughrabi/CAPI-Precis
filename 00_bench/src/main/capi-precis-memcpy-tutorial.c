...
// File   : capi-precis-memcpy-tutorial.c
...
main (int argc, char **argv)
{
    ...
    struct DataArraysTut *dataArraysTut = newDataArraysTut(&arguments);

    printf("*-----------------------------------------------------*\n");
    printf("| %-30s %-20u | \n", "Populating Data Arrays (Seed)", 27491095);
    printf(" -----------------------------------------------------\n");

    initializeDataArraysTut(dataArraysTut);
    ...
    Start(timer);
	copyDataArraysTut(dataArraysTut, &arguments);
    Stop(timer);
    printf("| %-22s | %-27.20lf| \n","Time (Seconds)", Seconds(timer));

    double bandwidth_GB = (double)((double)(dataArraysTut->size)/(double)(1024*1024*256))/Seconds(timer);  //GB/s
    double bandwidth_MB = (double)((double)(dataArraysTut->size)/(double)(1024*256))/Seconds(timer); //MB/s

    printf("| %-22s | %-27.20lf| \n","BandWidth MB/s", bandwidth_MB);
    printf("| %-22s | %-27.20lf| \n","BandWidth GB/s", bandwidth_GB);

    uint64_t missmatch = 0;
    missmatch = compareDataArraysTut(dataArraysTut);
