...
int main(void)
{
    ...
    wed.size_send = fake_target;
    if(setenv("CAPI_DEVICE", "/dev/cxl/test-afu", 1))
        return EXIT_FAILURE;
    setupAFU(&afu, &wed);

    if(strcmp(fake_open_path, "/dev/cxl/test-afu"))
        return EXIT_FAILURE;
